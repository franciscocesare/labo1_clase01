...
int utn_getString(char* pResultado,char* msg,char* msgError,int minimo,int maximo,int reintentos)
{
    ...
    strncpy(pResultado,bufferStr,4000);///va a copiar el buffer en pResultado, hasta el limite. aca 4000
...
/** \brief buscar lugar libre en un array
 *
 * \param matriz de array 1[cajonera] [pos de la cajon]
 * \param len cantidad de posiciones del Array
...
)
{
    ...
    for (i=0; i<len; i ++)
    {
       if (strcmp(pNombre, lista [i]))==0 ///funcion para comparar
    {
        *pIndex = i;
        ret=0;
        break;
