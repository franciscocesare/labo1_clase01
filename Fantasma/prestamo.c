...
int prestamo_alta(Prestamo array[], int size, int* contadorID)      ///cambiar prestamo
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            printf("\n Posicion: %d\n ID: %d\n varString: %s\n varLongString: %s",
                   posicion, array[posicion].idUnico,array[posicion].varString,array[posicion].varLongString);
            retorno=0;
        }
    }
    return retorno;
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d \n varString: %s\n varLongString: %s",
                       array[i].idUnico,array[i].varString,array[i].varLongString);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
